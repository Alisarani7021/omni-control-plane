...
| `/cleanip` · «💎 آی‌پی تمیز» | رادار IP تمیز (فقط از گزارش واقعی کلاینت‌ها) |
| `/map` · «🗺 نت ملی» | نقشهٔ زندهٔ سانسور + ثبت گزارش ناشناس |
| `/whitehole` · «🌪️ WhiteHole» | انتشار/پاک‌کردن هددراپ TXT در Zone خود کاربر |
| `/donate` · «🎁 اهدای AI» | صف اهدای کلید AI (رمزنگاری‌شده، با رضایت، قابل پس‌گیری) |
| `/health` · `/usage` | سلامت نودها از `agent_reports` و شمارش‌های واقعی D1 |
...
## ۷. حالت C (تک‌ورکر) و بخش‌های منتقل‌شدهٔ `worker.js`

اگر `worker.js` را کنار می‌گذارید، لازم نیست منطق ربات آن را نگه دارید: رادار IP تمیز،
نقشهٔ سانسور، هددراپ WhiteHole، استخر اهدای AI، سلامت نودها و شمارش‌های پنل حالا در همین
ریپو پیاده شده‌اند (نگاشت کامل، داده‌ها و موارد عمداً منتقل‌نشده در
`docs/PANEL-SECTIONS-FA.md`). فقط یادتان باشد:

1. پیش از هر چیز توکن رباتی که در سورس `worker.js` سفت (hardcoded) شده بود را در
   @BotFather باطل کنید؛ ربات V13 فقط از secret خودش (`TELEGRAM_BOT_TOKEN`) استفاده می‌کند.
2. مایگریشن `0004_panel_sections.sql` را اعمال کنید و
   `node scripts/set-telegram-webhook.mjs` را دوباره اجرا کنید تا دستورهای جدید ثبت شوند.
3. دو جریان «ثبت گزارش» و «ارسال کلید اهدایی» با متن آزاد کار می‌کنند، پس وب‌هوک باید روی
   همین کنترل‌پلین ثبت باشد (حالت A).
