...
export function faErrorMessage(error: unknown): string | null {
  if (error instanceof HttpError) {
    switch (error.code) {
      ...
      case "no_healthy_resolver":
        return "هنوز هیچ DNS سالمی از اسکنر پیدا نشده؛ اول در مرکز DNS «🔎 یافتن DNS سالم (اسکن رنج)» را بزنید و بگذارید حداقل یک «سالم» ثبت شود، بعد انتشار TXT معنا دارد.";
      default:
...
async function publishMasterDiscovery(env: Env, principal: SessionPrincipal, connectionId: string): Promise<{ name: string; count: number }> {
  const healthy = await healthyResolvers(env, principal.tenantId);
  if (healthy.length === 0) {
    throw new HttpError(409, "no_healthy_resolver", "No healthy resolver found yet; run a range scan first");
  }
  const connection = await getConnection(env, connectionId, principal.tenantId);
