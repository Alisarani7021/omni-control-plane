...
- Daily RIPE/IRNIC national-range intelligence (`src/geoip-ir.ts`): cron snapshots `delegated-irnic-extended-latest`, diffs against the previous day, keeps 14 days, and renders «منبع RIPE/IRNIC · +N رنج · -M رنج · امروز» with the snapshot sha256; a public sing-box v2 rule-set is served at `GET /api/v1/geoip-ir.json`.
